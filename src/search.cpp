                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (pos.capture_or_promotion(ttMove) && depth >= 5)
                    captureHistory[movedPiece][to_sq(ttMove)][type_of(pos.piece_on(to_sq(ttMove)))] << stat_bonus(depth) * 9 / 10;
            ...
            {
                int penalty = -stat_bonus(depth) / 10;
                thisThread->mainHistory[us][from_to(ttMove)] << penalty;
