...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score RookNearQueen       = S( 10,  10);
  ...
  Score Evaluation<T>::pieces() {
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == ROOK)
        {
            ...
            // Bonus for rook near same file as queen if RookOnQueenFile does not apply
            else if (adjacent_files_bb(s) & pos.pieces(QUEEN))
                score += RookNearQueen;
